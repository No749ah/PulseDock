...
# Detach stdin and ignore terminal hangups so the server survives the restart
# command's shell exiting (including PTY-backed heartbeat runners).
nohup setsid env NODE_ENV=production NODE_OPTIONS="--max-old-space-size=512" PORT="$WEB_PORT" \
  npx next start -H 0.0.0.0 -p "$WEB_PORT" \
  >> "$LOG_DIR/pulsedock_web_prod.log" 2>&1 < /dev/null &
WEB_PID=$!

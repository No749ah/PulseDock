...
export default function DashboardPage() {
  ...
  const loadDashboard = useCallback(async (silent = false) => {
    try {
      ...
      // Non-critical: fetch SLO summary, health timeline, active incidents (won't block main render)
      ...
      api<{ timeline: HealthTimelineEntry[] }>("/v1/dashboard/health-timeline?days=30")
        ...
        .catch(() => {});
      api<{ id: string; title: string; status: string; severity: string; createdAt: string; monitors: { id: string; name: string }[] }[]>("/v1/incidents?status=INVESTIGATING,IDENTIFIED,MONITORING&limit=10")
          ...
          <FadeIn>
            <div className="rounded-xl border border-danger/30 bg-danger/5 overflow-hidden">
              <div className="flex items-center gap-2 px-4 py-3 border-b border-danger/20 bg-danger/10">
                ...
                <span className="text-sm font-semibold text-danger">{activeIncidents.length} Active Incident{activeIncidents.length !== 1 ? "s" : ""}</span>
              </div>
              <div className="divide-y divide-border/50">
                {activeIncidents.map((inc) => (
                  <Link key={inc.id} href={`/incidents/${inc.id}`} className="flex items-center gap-4 px-4 py-3 hover:bg-surface-elevated/50 transition-colors group">
                    ...
                    <span className="text-sm font-medium text-text-primary flex-1 truncate group-hover:text-accent transition-colors">{inc.title}</span>
                    ...
                    {inc.monitors.length > 0 && (
                      <span className="text-xs text-text-secondary shrink-0 hidden sm:block">{inc.monitors.map(m => m.name).join(", ")}</span>
                    )}
                  </Link>
                ))}
              </div>

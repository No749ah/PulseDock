...
LIMIT_HARD_CAP_SECONDS="${FRONTEND_AUDIT_LIMIT_HARD_CAP_SECONDS:-86400}"
LIMIT_HARD_CAP_RETRIES="${FRONTEND_AUDIT_LIMIT_HARD_CAP_RETRIES:-1000}"
...
validate_positive_integer "FRONTEND_AUDIT_LIMIT_HARD_CAP_SECONDS" "$LIMIT_HARD_CAP_SECONDS"
validate_positive_integer "FRONTEND_AUDIT_LIMIT_HARD_CAP_RETRIES" "$LIMIT_HARD_CAP_RETRIES"
...
if [[ "$REQUEST_TIMEOUT_SECONDS_LIMIT" -gt "$LIMIT_HARD_CAP_SECONDS" ]]; then
  echo "FRONTEND_AUDIT_REQUEST_TIMEOUT_SECONDS_LIMIT must be <= FRONTEND_AUDIT_LIMIT_HARD_CAP_SECONDS (got: $REQUEST_TIMEOUT_SECONDS_LIMIT > $LIMIT_HARD_CAP_SECONDS)" >&2
  exit 1
fi

if [[ "$CONNECT_TIMEOUT_SECONDS_LIMIT" -gt "$LIMIT_HARD_CAP_SECONDS" ]]; then
  echo "FRONTEND_AUDIT_CONNECT_TIMEOUT_SECONDS_LIMIT must be <= FRONTEND_AUDIT_LIMIT_HARD_CAP_SECONDS (got: $CONNECT_TIMEOUT_SECONDS_LIMIT > $LIMIT_HARD_CAP_SECONDS)" >&2
  exit 1
fi

if [[ "$MAX_RETRY_DELAY_SECONDS_LIMIT" -gt "$LIMIT_HARD_CAP_SECONDS" ]]; then
  echo "FRONTEND_AUDIT_MAX_RETRY_DELAY_SECONDS_LIMIT must be <= FRONTEND_AUDIT_LIMIT_HARD_CAP_SECONDS (got: $MAX_RETRY_DELAY_SECONDS_LIMIT > $LIMIT_HARD_CAP_SECONDS)" >&2
  exit 1
fi

if [[ "$MAX_RETRIES_LIMIT" -gt "$LIMIT_HARD_CAP_RETRIES" ]]; then
  echo "FRONTEND_AUDIT_MAX_RETRIES_LIMIT must be <= FRONTEND_AUDIT_LIMIT_HARD_CAP_RETRIES (got: $MAX_RETRIES_LIMIT > $LIMIT_HARD_CAP_RETRIES)" >&2
  exit 1
fi

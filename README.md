<div align="center">
  ...
  <p>
    ...
    <img src="https://img.shields.io/badge/tests-5300%2B%20passing-brightgreen" alt="Tests: 5300+ passing" />
...
- Track version updates for **5000+ self-hosted tools** — auto-configured, no manual setup
...
├── packages/
│   ├── tool-registry/ # 5009+ pre-configured tool definitions
...
npm run test              # Run all tests (5300+ tests)
...
PulseDock has **5300+ tests** across the full stack:

```bash
npm run test                        # All tests
npm run test -w @pulsedock/api      # API tests only (4400+ API tests)
npm run test -w @pulsedock/agent    # Agent tests only
npm run test -w @pulsedock/cli      # CLI tests only
```

Coverage: 196 test files, TypeScript strict mode throughout, zero `any` types.

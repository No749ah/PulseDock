...
export default function HomePage() {
  ...
  return (
    <main style={{ maxWidth: 1200, margin: '0 auto', padding: 'clamp(12px, 3vw, 24px)' }}>
      <Card withBorder radius="xl" p="clamp(16px, 4vw, 32px)" mb="md" style={{ background: 'rgba(14,20,40,0.75)', backdropFilter: 'blur(10px)' }}>
        <Stack gap="md">
          <Group gap="xs">
            <Badge color="indigo" variant="light">PulseDock Enterprise</Badge>
            <Badge color="cyan" variant="light">NestJS + Next.js</Badge>
          </Group>
          <Title order={1} style={{ fontSize: 'clamp(30px, 6vw, 52px)', lineHeight: 1.05, letterSpacing: '-0.03em', textWrap: 'balance' }}>
          ...
          <Group gap="sm" wrap="wrap">
            <Button color="teal" size="md" style={{ flex: '1 1 210px' }} onClick={() => router.push('/login')}>Get Started</Button>
            <Button color="teal" variant="light" size="md" style={{ flex: '1 1 210px' }} onClick={() => router.push('/dashboard')}>Open Dashboard</Button>
          </Group>
        </Stack>
      </Card>
